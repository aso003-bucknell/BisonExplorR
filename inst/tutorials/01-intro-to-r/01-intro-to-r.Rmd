...
organizing observations into tidy tables — one row per observation, one column per
...
![The RStudio window divided into four panes: Source, Console, Environment, and Files/Plots/Packages/Help/Viewer.](images/posit1.png){width="100%"}
...
quiz(
  ...
  question("Where does R display a plot after you run plotting code?",
    answer("Source pane"),
    answer("Console pane"),
    answer("Plots pane", correct = TRUE, message = "Plots appear in the Plots pane."),
...
In the field you sampled goldenrod in **neighbor-matched pairs** — each galled stem
...
quiz(
  ...
  question('A column containing the values "galled" and "ungalled" would be what data type in R?',
    answer("numeric"),
    answer("logical"),
    answer("character", correct = TRUE, message = 'Text values are stored as the character type.'),
    allow_retry = TRUE
  ),
  question('One student typed stem widths as "5.3 mm" instead of 5.3. What happens to that column in R?',
    ...
    answer("R reads the whole column as character, so it cannot be averaged", correct = TRUE,
           message = "One bad cell changes the type of the entire column, and nothing warns you."),
...
`goldenrod`. It holds 15 neighbor-matched pairs — 30 stems in total — with stem width,
...
grade_this({
  pass_if(
    ...
    "Correct — `head(goldenrod)` shows the first six rows. Notice the rows come in pairs: each galled stem sits next to its matched neighbor."
  )
...
quiz(
  ...
  question("A student counted inflorescence shoots on a stem and found none. What belongs in the `n_inflorescence_shoots` cell?",
    ...
    answer("0", correct = TRUE,
           message = "The measurement was taken and the answer was zero. That is data, and it must not become NA."),
    answer("NA"),
    answer("A dash"),
    allow_retry = TRUE
  ),
  question("A stem snapped before it could be measured. What belongs in the cell?",
    ...
    answer("Leave it blank, and give the reason in a notes column", correct = TRUE,
           message = "R reads a blank as NA, which is exactly right — and the notes column records why, so it can be recovered later."),
...
quiz(
  ...
  question("Question 1: How many observations (rows) does the goldenrod dataset have?",
    ...
    answer("30", correct = TRUE, message = "The top line indicates there are 30 observations — 15 pairs, two stems each."),
    answer("60"),
    allow_retry = TRUE
  ),
  question("Question 2: In your str(goldenrod) output, what data type is the gall_status variable?",
    answer("numeric"),
    answer("logical"),
    answer("character", correct = TRUE, message = "It is listed as a character (chr) variable."),
    allow_retry = TRUE
  ),
  question("Question 3: What data type is stem_width_mm?",
    answer("character"),
    answer("logical"),
    answer("numeric", correct = TRUE, message = "In str() output, numeric variables usually appear as 'num'."),
    allow_retry = TRUE
  ),
  question("Question 4: Why does the dataset need a pair_id column at all?",
    answer("To count how many stems were sampled"),
    answer("To record which galled stem was matched with which ungalled neighbor", correct = TRUE,
           message = "Without pair_id the matching you did in the field is lost, and the wrong statistical test runs silently."),
...
Every value in that column is either `galled` or `ungalled`, spelled and capitalized the
...
**Module 2** will show you how to import a data file with `read.csv()` — starting with the goldenrod data you just explored, and eventually your own.
